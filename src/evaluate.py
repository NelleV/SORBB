...
def average_precision(search_file_names, positive_file_names,
                      ignore_file_names):
    sum_prec = 0.0
    correct = 0
    num_doc = 0
    ...
    for i, search_result in enumerate(search_file_names):
        if search_result in ignore_file_names:
            continue
        is_true = (search_result in positive_file_names)
        num_doc += 1
        if is_true:
            correct += 1
            sum_prec += float(correct) / (i + 1)

    return sum_prec / num_doc
